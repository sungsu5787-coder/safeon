...
- **요구**: "이 앱을 토스·애플·테슬라 조합으로 리디자인". 추천 조합 질문에 디자이너로서 **토스60·애플25·테슬라15** 비균등 역할 분담을 제시·채택.
  - 토스=골격(일상 사용성·카드·라운드·마이크로인터랙션), 애플=구조(여백·타이포 위계·KPI 큰 숫자), 테슬라=긴장감(위험·긴급만 ink 고대비).
  - 근거: 매일 현장에서 쓰는 안전 PWA. 사용성 최우선이라 애플·테슬라 베이스는 부적합(애플=전시용, 테슬라 다크=옥외 가독성 최악). 토스 골격에 둘을 조미료로.
- **전략 — 6551줄 통째 재작성 안 함**: 라이브 안전 앱 파손 위험. CSS가 토큰 기반이라 `:root` 교체가 최고 레버리지. → 토큰 레이어 + 시그니처 컴포넌트만 surgical. HTML 구조 보존.
...
- **치명버그(검증으로 발견)**: `_printOfficialForm()`이 `type` 변수를 선언 없이 3곳(사고성재해·사망여부)에서 사용 → 인쇄 실행 시 `ReferenceError: type is not defined`로 산업재해·중대재해 인쇄가 **완전 무출력**이었음(v1.7.8~1.7.9 운영본 포함). `_printInternalReport`처럼 `const type = this.getSelectedType()` 추가로 해결. 실제 accident.js를 로드하는 하니스(_test_form30.html)로 재현·수정 확인.
- **사용자 결정**: ① 비공식 체크박스 그리드(재해유형·상해부위·상해종류)는 공식 양식에 없으므로 제거하고 자유기입+하단 분류자란으로 정리. ② 건설업 전용 칸은 제외(제조·사출 현장).
- **공식양식 근거**: 산업안전보건법 시행규칙 별지 제30호서식. law.go.kr·moel.go.kr 검색 교차확인. 구조 = ①사업장정보(산재관리번호·사업자등록번호·사업장명·근로자수·업종·소재지·원수급/파견) ②재해자정보(성명·주민번호·성별·국적·체류자격·직업·입사일·근속·고용형태·근무형태·상해종류·상해부위·휴업예상일수·사망여부) ③재해발생개요및원인(발생일시·장소·작업유형·당시상황·발생원인) ④재발방지계획 / 작성자·전화·작성일·사업주서명·근로자대표(재해자)서명·"○○지방고용노동청장(지청장) 귀하" / 하단 "재해 분류자 기입란(※사업장 미작성)": 발생형태·기인물·작업지역공정·작업내용 코드.
- **입력 매핑**: 성명←injured-name, 상해부위←injured-part, 상해종류←injured-level, 직업←accident-dept, 발생일시←date+time(요일 자동계산), 발생장소←location, 당시상황←content, 재해발생원인←cause, 재발방지←prevention, 작성자←reporter, 사업주←WorkplaceInfo[0].ceo, 사업장명/근로자수/소재지←WorkplaceInfo[0]. 사망여부 ☑사망 = type==='serious'.
- **남은 한계**: 산재관리번호·사업자등록번호·체류자격·근무형태·휴업예상일수·작성자전화·근로자대표 서명은 데이터 소스가 없어 빈칸(수기 기입). 분류자란도 빈칸(노동관서 작성). 제조 현장 기준이라 건설업 칸 미포함.
