...
- [x] Phase 5. 컴포넌트 점검 — 주요 컴포넌트(메뉴카드·버튼)는 이미 TDS 품질(radius 토큰·scale 탭피드백·transition 완비) → 무리한 재구성은 해가 되어 보류.
- [x] Phase 5a. 단일값 radius 70곳(10/16/20px) → 토큰 참조 통일 (렌더 동일, 무해). 캐시 미변경 배포.
- [ ] 배포 후 실제 화면 회귀 확인 (보라/틸 평탄화로 인한 단조로움 점검)

## Phase 5 판단 기록
"해가 되지 않는 방향" 요청 → 보이는 변화 = 위험으로 간주. 섀도는 토큰과 정확히 일치하는 값이 없어 제외. 어중간한 radius(8·12·14px)도 제외. 토큰과 픽셀값이 동일한 단일 radius만 토큰화.
