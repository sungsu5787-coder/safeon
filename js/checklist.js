...
const Checklist = {
  ...
  init() {
    ...
    // Photo upload
    const photoInput = document.getElementById('check-photo-input');
    if (photoInput) photoInput.addEventListener('change', e => this.handlePhotos(e));

    const uploadArea = document.getElementById('check-upload-area');
    ...
    container.innerHTML = this.photos.map((p, i) => `
      <div class="photo-thumb">
        <img src="${p.data}" alt="${App.escapeHtml(p.name)}"
             onclick="Checklist.viewPhoto(${i})">
        <button type="button" class="photo-remove"
                onclick="event.stopPropagation();Checklist.removePhoto(${i})"
                title="삭제">×</button>
      </div>
    `).join('');
  },
  ...
  async save() {
    ...
    const ok = await App.confirm(
      `<b>${typeNames[type]}</b><br>${date} · ${location || '장소 미입력'}${failTxt}${photoTxt}`,
  ...
  resetForm() {
    ...
    this.results = {};
    this.photos = [];
    const prev = document.getElementById('check-photo-preview');
