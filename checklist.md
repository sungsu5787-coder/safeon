...
- [x] 사용자 결정: 체크박스 그리드 제거(공식대로 자유기입+분류자란), 건설업 칸 제외
- [x] ① 사업장정보: 산재관리번호·사업자등록번호 칸 추가, 대표자→사업주 서명란으로 이동, 건설업 칸 제외
- [x] ② 재해자정보: 주민등록번호·체류자격·근무형태 추가, 고용형태 공식 선택지, 상해종류/상해부위 자유기입, 휴업예상일수·사망여부
- [x] ③ 재해발생개요및원인: 발생일시(요일자동)·발생장소·재해관련작업유형·당시상황(content)·재해발생원인(cause)
- [x] ④ 재발방지계획(prevention)
- [x] 작성자 성명/전화·작성일·사업주 서명·근로자대표(재해자) 서명·"○○지방고용노동청장(지청장) 귀하"
- [x] 하단 재해 분류자 기입란(발생형태·기인물·작업지역공정·작업내용, "사업장 미작성" 명시)
- [x] 현장사진은 [첨부]로 양식 하단 분리
- [x] 실제 accident.js 로드 하니스로 렌더링 검증(산업재해/중대재해 2종)
- [ ] 라이브: 실기기 인쇄 출력 확인(사용자)
